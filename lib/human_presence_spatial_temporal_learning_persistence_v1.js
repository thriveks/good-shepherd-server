...
const TABLE_SQL = `
  CREATE TABLE IF NOT EXISTS
    human_presence_spatial_temporal_snapshots (
      ...
      spatial_temporal_learning_version TEXT NOT NULL,
      spatial_state_learning_version TEXT NOT NULL,
      node_id TEXT NOT NULL,
      temporal_learning_payload JSONB NOT NULL,
      evidence_received_at TIMESTAMPTZ NOT NULL,
      temporal_snapshot_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),

      PRIMARY KEY (
        evidence_event_id,
        spatial_temporal_learning_version
      )
    );

  CREATE INDEX IF NOT EXISTS
    human_presence_spatial_temporal_snapshots_node_time_idx
  ON human_presence_spatial_temporal_snapshots (
    node_id,
    evidence_received_at DESC
  );
`;

async function ensureHumanPresenceSpatialTemporalLearningV1(
  db
) {
  await db.query(
    TABLE_SQL
...
function requireCurrent(current) {
  if (
    ...
    throw new Error(
      "Spatial Temporal Learning evidence identity required"
    );
  }
}
...
async function buildAndPersistHumanPresenceSpatialTemporalLearningV1(
  db,
  current
) {
  requireCurrent(
  ...
  const result =
    await db.query(
      `
        ...
        DO NOTHING
        RETURNING temporal_snapshot_at
      `,
      [
        current.evidence_event_id,
        VERSION,
  ...
  return {
    ...
    persistence: {
      ...
      spatialTemporalLearningVersion:
        VERSION
    }
  };
}
