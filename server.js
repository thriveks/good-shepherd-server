...
// ============================================================================
// GOOD SHEPHERD — HUMAN PRESENCE INTERPRETATION v1
// Analytical/descriptive layer only. No alerts, cases, dispatch, or emergency
// classification are generated by this layer.
// ============================================================================
const {
...
async function initializeDatabase() {
  const schemaBaselineVersion =
    "2026-09-06-production-schema-baseline-v1";
  ...
  const migrationBaseline =
    await pool.query(
      `
        ...
        LIMIT 1
      `,
      [schemaBaselineVersion]
    );

  if (migrationBaseline.rowCount !== 1) {
    throw new Error(
      `Required database schema baseline ${schemaBaselineVersion} is not applied`
    );
  }
...
app.get("/ai/human-presence-learning", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      grouped AS (
        SELECT
          authoritative_resident_id AS "residentId",
          authoritative_resident_name AS "residentName",
          authoritative_room_or_location AS "roomOrLocation",
          COUNT(*)::int AS "observations",
          ROUND(
      ...
      SELECT *
      FROM grouped
      ORDER BY "lastObservationAt" DESC
      `,
      [residentId]
    );
    ...
    const locations = result.rows.map((row) => {
      ...
      const hoursObserved = Number(row.hoursObserved || 0);
      const daysObserved = Number(row.daysObserved || 0);

      const gates = {
        observations: observations >= requirements.observations,
        hours: hoursObserved >= requirements.hours,
        calendarDays: daysObserved >= requirements.calendarDays
      };
      ...
      return {
        ...
        status: ready ? "ready_for_analysis" : "collecting",
        statusLabel: ready ? "Learning Complete" : "Learning your routine",
        observations,
        hoursObserved,
        daysObserved,
    ...
    console.error("Human Presence learning status failed:", error);
    return res.status(500).json({
      success: false,
      error: "Failed to load Human Presence learning status"
    });
  }
});
...
async function interpretAndPersistHumanPresenceCandidateV1(nodeId, payload) {
  const eventId = String(payload?.eventId || "").trim();

  if (!eventId) {
    throw new Error("Interpretation v1 missing candidate evidence eventId");
  }

  // Consume the already-persisted immutable raw evidence row.
  ...
  if (!evidence) {
    throw new Error(
      `Interpretation v1 cannot locate persisted evidence ${eventId}`
    );
  }

  const rawPayload =
    evidence.eventPayload &&
    typeof evidence.eventPayload === "object"
      ? evidence.eventPayload
      : {};
  ...
  // Exact source_key is the authoritative sensor lookup.
  const sensorResult = evidence.sourceKey
    ? await pool.query(
        `
          SELECT
            id,
  ...
  const authority =
    resolveHumanPresenceInterpretationAuthorityV1({
      evidence: {
        ...
        residentName: evidence.residentName,
        locationName: evidence.locationName,
        softwareVersion: evidence.softwareVersion
      },
      sensor,
      resident,
      node
    ...
    [
      ...
      JSON.stringify({
        ...
        authorityResolutionStatus:
          authority.status,
        firmwareProvenance:
          authority.firmwareProvenance || null,
        nodeProvenance:
          authority.nodeProvenance || null,
        descriptiveOnly: true,
  ...
  console.log(
    "Human Presence Interpretation v1:",
    evidence.eventId,
    result.rowCount > 0 ? "persisted" : "already_exists",
    authority.status
  );
...
async function ingestMqttV2Event(nodeId, payload) {
  ...
  if (eventType === "high_resolution_activity_evidence") {
    ...
    console.log(
      ...
      persistence.eventId,
      engineeringFeatureResult
        ? engineeringFeatureResult.persistence.engineeringFeatureVersion
        : null
    );
  ...
  if (eventType === "candidate_history_evidence") {
    ...
    if (
      ...
      const episodeProfileTemporalContextAnalysisResult =
      ...
      console.log(
        "Human Presence Episode Profile Temporal Context Analysis v1:",
        evidenceResult.eventId,
